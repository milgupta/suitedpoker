...
export default function Home() {
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* The hero loop. Muted, looping, and 81KB — the frequency bar filling in
          does more than thirty words of copy about "seeing the whole strategy".
          The poster means the slot is never blank on a slow connection, and the
          fixed aspect ratio means it never shifts layout (CLS is 0). */}
      <section className="border-border border-t">
        <div className="mx-auto max-w-(--container-marketing) px-6 py-16">
          <video
            ...
            poster="/hero/drill-loop.jpg"
            autoPlay
            muted
            loop
            playsInline
            preload="none"
            aria-label="A hand appears, an action is chosen, and the full strategy is revealed"
          >
            <source src="/hero/drill-loop.webm" type="video/webm" />
            <source src="/hero/drill-loop.mp4" type="video/mp4" />
          </video>
        </div>
      </section>
          ...
          <ol className="mt-10 grid gap-6 sm:grid-cols-3">
            {[
              ...
              <li key={title} className="border-border bg-surface-1 rounded-lg border p-5">
                ...
                <p className="text-text-secondary text-body-md mt-2">{body}</p>
                {/* Real screenshots, regenerated by `npm run screenshots`. AVIF
                    with a WebP fallback — these are flat UI on a dark ground,
                    which AVIF compresses about 82% smaller than the PNG. */}
                <picture>
                  <source srcSet={`/screenshots/${shot}.avif`} type="image/avif" />
                  <source srcSet={`/screenshots/${shot}.webp`} type="image/webp" />
                  <img
